...
import { activationType, CostTypes, ReiatsuTypes, triggerClauseType } from "../../enums"
...
export class EnergyRemoval extends Effect {
    ...
    public generateToolTip() {
        if(this.triggerClause === triggerClauseType.None) {
            this.message = `This character will lose ${this.value} ${ReiatsuTypes[this.energyType]} reiatsu`
        } else if (this.triggerClause === triggerClauseType.IfTargeted) {
            this.message = `If this character is targeted by a new skill they'll lose ${this.value} ${ReiatsuTypes[this.energyType]} reiatsu`
        }
    }
